...
const ENABLED_DAILY = ['grants_gov_simpler', 'grants_gov_full', 'california_grants'];
...
test('ingestion is idempotent across repeated runs', async () => {
  const count = await withDatabase(async (db) => {
    ...
    await worker.run({ db, fetchImpl: createFakeFetch(), env: ENV });
    await worker.run({ db, fetchImpl: createFakeFetch(), env: ENV });
...
test('normalization maps every source shape into grants_normalized', async () => {
  ...
  const full = rows.find((r) => r.source_id === 'grants_gov_full' && r.external_id === '1');
  assert.strictEqual(full.posted_date, '2026-02-10', 'MM/DD/YYYY converts to ISO');
...
test('scheduler runs both cadences and reports ingestion counts', async () => {
  ...
  const daily = await engine.runCadence('daily', { databasePath, fetchImpl, env: ENV });
  assert.deepStrictEqual(daily.ingestion.failures, []);
  assert.strictEqual(daily.ingestion.results.length, 3);

  const weekly = await engine.runCadence('weekly', { databasePath, fetchImpl, env: ENV });
  ...
  assert.strictEqual(stats.grants_raw, 19, '1 simpler + 7 full + 1 CA + 2 DoD + 2 NASA + 6 agency');
  assert.strictEqual(stats.grants_normalized, 19);
  assert.strictEqual(stats.grant_topics, 2);
});

test('a failing source is isolated and does not stop the cadence', async () => {
  const fetchImpl = async (url, init) => {
    if (url.includes('simpler.grants.gov')) return { ok: false, status: 503, json: async () => ({}), text: async () => '' };
    return createFakeFetch()(url, init);
  };

  const daily = await engine.runCadence('daily', { databasePath: tempDbPath('failure'), fetchImpl, env: ENV });
  ...
  assert.strictEqual(daily.ingestion.results.length, 2);
});
