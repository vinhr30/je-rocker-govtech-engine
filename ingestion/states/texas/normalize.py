...
def normalize_records(reset: bool = False) -> list[dict]:
    ...
    conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_tx_normalized_url ON normalized_listings(url)")
    conn.commit()
    ...
    for row in rows:
        ...
        payload = {
            ...
            "commodity_codes": json.dumps(normalize_commodity_codes(row["commodity_codes_raw"]), ensure_ascii=False),
